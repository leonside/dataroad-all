...
public class JsonJobSchemaParser implements JobSchemaParser {
    ...
    public JobConfigs parserJSONPath(String path) throws IOException, URISyntaxException {
        JobConfigs job = null;
        if(path.startsWith(JobCoreConstants.JOBSCHEMA_PATH_PREFIX_CLASSPATH)) {
            path = path.replaceAll(JobCoreConstants.JOBSCHEMA_PATH_PREFIX_CLASSPATH,"");
        ...
        }else if(path.startsWith(JobCoreConstants.JOBSCHEMA_PATH_PREFIX_FILESYSTEM)){
            path = path.replaceAll(JobCoreConstants.JOBSCHEMA_PATH_PREFIX_FILESYSTEM,"");
