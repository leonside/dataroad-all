 ...
 */
@PublicEvolving
public class GenericOutputFormatSinkFunction<IN> extends OutputFormatSinkFunction<IN> implements CheckpointedFunction {

    private static final long serialVersionUID = 1L;

    private static final Logger LOG = LoggerFactory.getLogger(GenericOutputFormatSinkFunction.class);
    ...
    public GenericOutputFormatSinkFunction(OutputFormat<IN> format) {
        super(format);
        this.format = format;
    }
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        if (format instanceof GenericRichOutputFormat && formatStateMap != null){
            ((GenericRichOutputFormat) format).setRestoreState(formatStateMap.get(context.getIndexOfThisSubtask()));
        }
    ...
    public void snapshotState(FunctionSnapshotContext context) throws Exception {
        FormatState formatState = ((GenericRichOutputFormat) format).getFormatState();
        if (formatState != null){
