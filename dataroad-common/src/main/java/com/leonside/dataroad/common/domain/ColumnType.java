...
//        }
    ...
    public static ColumnType getType(String type){
        type = type.toUpperCase(Locale.ENGLISH);
//        if(type.contains(JobConfigConstants.LEFT_PARENTHESIS_SYMBOL)){
//            type = type.substring(0, type.indexOf(JobConfigConstants.LEFT_PARENTHESIS_SYMBOL));
//        }
//
//        //为了支持无符号类型  如 int unsigned
//        if(StringUtils.contains(type,JobConfigConstants.DATA_TYPE_UNSIGNED)){
//            type = type.replaceAll(JobConfigConstants.DATA_TYPE_UNSIGNED,"").trim();
//        }
